...
| `infrastructure/cloudformation-tool.yaml` | CloudFormation template for creating tool resources (Lambda, DynamoDB, IAM). |
| `infrastructure/cloudformation-testing.yaml` | CloudFormation template for creating testing resources (S3, IAM role). |
| `src/create_bug_report.py` | Lambda function implementation for storing bug reports in DynamoDB. |
| `data/online_shop_faq.md` | Fictional FAQ document embedded in prompt for answering customer queries. |
| `testing/generate-eval-dataset.py` | Script that runs flow tests and generates a JSONL file for Bedrock Evaluations. |
| `testing/flow-tests-template.json` | Template for developing test suites. |
| `solution/` | Reference solution folder for complete flow definitions, test prompts, and diagrams. |
